...
- [x] **Reverts** (ADR-0015, 2026-09-20) — a revert is a new balanced
      posting set mirroring the original by `transfer_id`, linked by
      `reverts`; originals immutable; authorized on the accounts it debits;
      never creates debt (no `force`); revert-of-revert supported; a
      `transfers` read model and `GET /v1/transfers/{id}`. ADR before code;
      an independent review pass fixed one MEDIUM (posted-hold `kind` was
      arrival-order dependent) and one LOW (the route was an existence
      oracle) before merge. Released as v0.9.0.
